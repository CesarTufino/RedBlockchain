...
public class TestV5Nodo1 {

    public static void main(String[] args) throws IOException {
        NodoTradicionalGateway nodoTradicionalGateway = new NodoTradicionalGateway(1, Direccion.DIRECCION_1);
        Entrada hiloEntrada = new Entrada(nodoTradicionalGateway);
        hiloEntrada.start();
        nodoTradicionalGateway.buscarRed();
        while (true) {
            if (nodoTradicionalGateway.comprobarCantidadMinimaDeNodos()) break;
            System.out.print("");
        }
        try {
            Thread.sleep(1000);
        ...
        for (int i = 0; i < 700; i++) {
            ...
            if (a == 1)
                nodoTradicionalGateway.enviarDinero(1.23, Direccion.DIRECCION_2.getDireccionIP());
            else
                nodoTradicionalGateway.enviarDinero(3.47, Direccion.DIRECCION_3.getDireccionIP());
            try {
                Thread.sleep(3000);
            } catch (InterruptedException e) {
