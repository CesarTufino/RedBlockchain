...
public class TestV6Nodo4 {

    public static void main(String[] args) throws IOException {
        NodoMultipleGateway nodoMultipleGateway = new NodoMultipleGateway(4, Direccion.DIRECCION_4, true);
        Entrada hiloEntrada = new Entrada(nodoMultipleGateway);
        hiloEntrada.start();
        nodoMultipleGateway.buscarRed();
        while (true) {
            if (nodoMultipleGateway.comprobarCantidadMinimaDeNodos())
                break;
            System.out.print("");
        }
        try {
        ...
        for (int i = 0; i < 700; i++) {
            ...
            if (a == 1)
                nodoMultipleGateway.enviarDinero(1.23, Direccion.DIRECCION_1.getDireccionIP(), Tipo.LOGICO1);
            else
        ...
        while (true) {
            if (nodoMultipleGateway.getRed().NB_OF_BLOCK_OF_TYPE1_CREATED.size() + nodoMultipleGateway.getRed().NB_OF_BLOCK_OF_TYPE2_CREATED.size() - 2 == MaximoDeBloques.MAX.getCantidad()) {
                try {
